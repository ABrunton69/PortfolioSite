document.addEventListener('DOMContentLoaded', () => {
    ...
    // Automatically trigger the transition after a delay
    setTimeout(() => {
        // Start fading the splash screen out
        splashScreen.classList.add('fade-out');

        // After the fade-out animation completes...
        setTimeout(() => {
            // Remove splash screen from view
            splashScreen.style.display = 'none';

            // Show main content
            header.classList.remove('hidden-initial');
            main.classList.remove('hidden-initial');
            footer.classList.remove('hidden-initial');
            
            // Re-enable scrolling
            document.body.classList.remove('no-scroll');
        }, 500); // This delay should match your CSS fade-out duration

    }, 1500); // Total time splash screen is visible before it starts fading
});
